...
bool is_mom_friend_son = 0, is_new_storage = 0, discount_black_friday = 0, discount_sale = 0;

size_t count_sales = 0, count_changed_storage_times = 0, procent_discount = 0; double earned_money = 0.0;
std::string client;
...
void login(short attempt) {
    ...
    std::cout << "3. Авторы\n";
    std::cout << "4. Войти как покупатель\n";
    std::cout << "5. Выход из магазина\n";
    std::cin >> type_enter;
...
void discounts(std::size_t id, double product_price) {
    ...
    std::map<std::size_t,std::string> type_discounts;
    std::string select_discount = "", type_discount = "";
    type_discounts[1] = "Введите скидку на товар " + productName[id] + " от 50% до 80%:";
    type_discounts[2] = "Введите скидку на товар " + productName[id] + " от 20% до 30%";
    std::string select_discounts[]{"","Черная пятница","Распродажа на ноябрь"};
    for(std::size_t i = 1; i < 3; ++i) {
        std::cout << i << ") " << select_discounts[i] << "\n";
    }
    while(attempt < 10) {
        if(attempt >= 10) {
            std::cout << YELLOW << "Было использовано более 10 попыток на выбор скидки. Попробуйте позже" << RESET << std::endl;
            std::this_thread::sleep_for(std::chrono::seconds(300));
            attempt = 0;
        }
        std::cout << "Выберите тип скидки:\n";
        std::cin >> select_discount;
        if(std::stoi(select_discount) < 1 || std::stoi(select_discount) > 2) {
            std::cerr << RED << "Ошибка выбора скидки. Попробуйте еще раз" << RESET << std::endl;
            select_discount.clear();
            attempt++;
        } else {
            type_discount = select_discounts[std::stoi(select_discount)];
            std::cout << type_discounts.at(std::stoi(select_discount)) << "\n";
            attempt = 0;
            discount.clear();
            break;
        }
    }
    if(type_discount == "Черная пятница") {
        while(attempt < 10) {
            if(attempt >= 10) {
                std::cout << YELLOW << "Было использовано более 10 попыток на выбор скидки. Попробуйте позже" << RESET << std::endl;
                std::this_thread::sleep_for(std::chrono::seconds(300));
            ...
            if(std::stoi(discount) < 50 || std::stoi(discount) > 80) {
                std::cerr << RED << "Ошибка ввода скидки на товар.\nПопробуйте еще раз" << RESET << std::endl;
                attempt++;
            } else if(!std::isdigit(discount.at(0),std::locale("")) || !std::isdigit(discount.at(discount.size() - 1),std::locale(""))) {
                std::cerr << RED << "Ошибка ввода скидки на товар.\nПопробуйте еще раз" << RESET << std::endl;
                attempt++;
            } else {
                procent_discount = std::stoi(discount);
                double new_price = (product_price * std::stod(discount)) / 100.0;
                price[id] = new_price;
                discount_black_friday = 1;
...
void start(short attempt) {
    ...
    if (is_new_storage == 1) {
        ...
        for (std::size_t i = 0; i < set_product.size(); ++i) {
    ...
    auto find_id_product_2 = std::find(set_id.begin(), set_id.end(), std::stoi(select_id_product));
    auto find_from_first = std::find(id.begin(), id.end(), id.at(0));
    auto find_from_first_2 = std::find(set_id.begin(), set_id.end(), set_id.at(0));
    if (find_id_product == id.end() || find_id_product_2 == set_id.end()) {
        std::cerr << RED << "Такого товара с таким айди нет\nПопробуйте еще раз" << RESET << std::endl;
        attempt++;
        start(attempt);
    }
    if (std::stoi(count_product) > count[std::distance(find_from_first, find_id_product)] ||
        std::stoi(count_product) > set_count[std::distance(find_from_first_2, find_id_product_2)]) {
        ...
        set_count[std::distance(find_from_first_2, find_id_product_2)] -= std::stoi(count_product);
...
void new_storage(short attempt) {
    if (attempt >= 10) {
        std::cout << YELLOW << "Было использовано более 10 попыток внести новый товар на новый склад. Попробуйте еще раз" << std::endl;
        std::this_thread::sleep_for(std::chrono::seconds(300));
    }
    ...
    while (create < get_size) {
        std::cout << "Напишите айди для нового товара:\n";
        std::cin >> create_id;
    ...
    std::cout << LIGHT_BLUE << std::right << std::setw(62) << "Новый склад\n" << RESET;
    std::cout << "Айди" << "\t" << "Наименование товара" << "\t" << "Цена" << "\t" << "Количество" << std::endl;
